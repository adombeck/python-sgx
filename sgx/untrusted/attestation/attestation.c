...
void initialize_quoting_enclave(sgx_target_info_t** pp_qe_target_info)
{
    sgx_target_info_t* tmp = malloc(sizeof(sgx_target_info_t));
    memset(tmp, 0, sizeof(sgx_target_info_t));

    sgx_epid_group_id_t gid = {0};

    sgx_status_t ret = sgx_init_quote(tmp, &gid);
    if(ret != SGX_SUCCESS)
    {
        // XXX: Throw Python exception. See http://www.swig.org/Doc1.1/HTML/Exceptions.html
    ...
    *pp_qe_target_info = tmp;

    fprintf(stderr, "Successfully initialized quoting enclave\n");
    print_target_info(**pp_qe_target_info);
}
...
int main()
{
    ...
    initialize_quoting_enclave(&p_qe_target_info);
